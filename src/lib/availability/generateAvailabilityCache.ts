...
// // Utility to extract time string (HH:mm:ss) from a UTC datetime string
// function extractTimePartFromUTC(datetime: string): string {
//   return new Date(datetime).toISOString().substring(11, 19); // "HH:MM:SS"
// }

// // Combine a local date (YYYY-MM-DD) with a time part ("HH:mm:ss") and convert to UTC
...
//   const bufferBefore = availabilityConfig.bufferMinutesBefore || 0;
//   const bufferAfter = availabilityConfig.bufferMinutesAfter || 0;
...
//       const slotStartTime = extractTimePartFromUTC(slot.start); // "03:30:00"
//       const slotEndTime = extractTimePartFromUTC(slot.end);     // "09:30:00"
...
//           current.getTime() +
//             slotInterval * 60 * 1000 +
//             bufferBefore * 60 * 1000 +
//             bufferAfter * 60 * 1000
//         );

//         if (actualEnd > slotEnd) break;
